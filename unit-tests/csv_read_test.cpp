...
TEST(CsvRead, TestOpenInCtor)
{
	...
	io::Csv<std::string> csv1(std::string("100_Sales_Records.csv"),
		std::ios_base::in, true);
	EXPECT_TRUE(csv1.is_open());
}
...
TEST(CsvRead, TestOpen)
{
	...
	EXPECT_TRUE(csv.open(std::string("100_Sales_Records.csv"),
		std::ios_base::in, true));
...
TEST(CsvRead, TestHeader)
{
	...
	io::Csv<std::string, std::string> csv2(std::string("100_Sales_Records.csv"),
		std::ios_base::in, true);
...
{
	io::Csv<std::string, std::string, std::string, std::string, std::string, std::string,
		std::string, std::string, std::string, std::string, std::string, std::string, std::string,
		std::string, std::string, std::string, std::string> csv(std::string("100_Sales_Records.csv"),
			std::ios_base::in, true);
...
{
	io::Csv<std::string> csv(std::string("100_Sales_Records.csv"),
		std::ios_base::in, true);
	std::string region;
	csv.read_values(region);
	EXPECT_STREQ("Australia and Oceania", region.c_str());
}

TEST(CsvRead, TestMultipleValue)
{
	io::Csv<std::string, std::string, std::string> csv(std::string("100_Sales_Records.csv"),
		std::ios_base::in, true);
	std::string region, country, item_type;
	csv.read_values(region, country, item_type);
	EXPECT_STREQ("Australia and Oceania", region.c_str());
